# Messaging — ROC (draft)

_Internal drafting aid. Do not publish without review._

## Elevator pitch (internal)

ROC unifies our remote-facing operations so customers experience one coherent Marine Technologies response, and we run vessel-to-shore activities with clear roles and measurable quality.

## Boilerplate (external — pending approval)

**Remote Operations Center (ROC):** Marine Technologies’ coordinated shore-side organization for customer support, remote monitoring, and remote operational assistance—supported by disciplined handling of operational data from vessel to shore.

## Definitions

- **Remote monitoring:** Systematic observation of vessel-related operational signals per contract and product capabilities—not a substitute for regulatory watchkeeping on board unless explicitly defined elsewhere.

- **Remote operations support:** Structured assistance for remotely supervised functions within authorized boundaries.

## Words to avoid (until Legal/Commercial approves)

_Ask Legal for locale-specific rules—examples only:_ “autonomous,” “unmanned,” “guaranteed uptime” without qualification.

## Tagline ideas (optional)

- _TBD — shortlist with Marketing._
